...
[NOTE]
====
1. Check `LAUNCH` is correctly set to point to `mpix_launch_swift` in `run-workflow.sh`. (**Q: Where is `mpix_launch_swift` to be found?**)
2. export LD_LIBRARY_PATH to include evpath library if you need. (**Not clear what this means**)
----
