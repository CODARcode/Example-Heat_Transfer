...
. Install all APT packages in the <<_installation,Installation>> section above (or equivalent on your OS)
...
*** This is needed so that the ADIOS-linked programs can find EVPath
    at run time
. Run ./run-workflow.sh +
It should take 20 seconds or less, if it hangs,
kill it and check for error messages.
