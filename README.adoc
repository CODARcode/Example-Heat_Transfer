...
.. Determine the location of the code (spack find -p savanna) and cd there
...
.. Determine its location: spack find -p adios
.. Add to PATH env
. Run the workflow
+
.. Edit +Heat_Transfer/heat_transfer.xml+ to enable the `FLEXPATH` method. Ensure that the following text, near the end of the file, is not commented out:
----
