...
program heat_transfer
    ...
    if (rank==0) then
        print '("Rank 0 runtime: ",f12.3)', tend-tstart
        print '("Rank 0 io_time: ", f12.3)', io_total_time
    endif
...
subroutine usage()
    ...
    print *, "nx:     global array size in X dimension"
    print *, "ny:     global array size in Y dimension"
...
subroutine processArgs()
    ...
    character(len=256) :: npx_str, npy_str, gndx_str, gndy_str
    ...
    call getarg(4, gndx_str)
    call getarg(5, gndy_str)
    call getarg(6, steps_str)
    call getarg(7, checkpoints_str)
    call getarg(8, r_str)
    read (npx_str,'(i5)') npx
    read (npy_str,'(i5)') npy
    read (gndx_str,'(i6)') gndx
    read (gndy_str,'(i6)') gndy
